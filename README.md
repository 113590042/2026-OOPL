...
- 搜尋並分析 OpenSyobon 原始碼中的關卡、素材及 magic number 對應。
- 協助將舊版全域資料拆分成場景、實體、工廠與管理器。
- 依遊戲截圖定位素材錯配、碰撞範圍及關卡座標問題。
- 建立可指定關卡與座標的自動截圖、關卡稽核及 smoke test 工具。
- 協助整理註解、建置錯誤及重複程式碼。

AI 產生的修改仍需經過人工操作、截圖比對、編譯及遊戲測試；關卡行為與最終取捨由開發者確認。
